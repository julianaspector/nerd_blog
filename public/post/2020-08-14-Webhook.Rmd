...
1) I followed the [Setup with JSON Web Tokens (JWT) guide](https://developer.box.com/guides/applications/custom-apps/jwt-setup/) from Box to set up app. 
...
![Configuration settings for access level and application scopes](/img/configuration_settings.png)
...
![Associate app owner login ID with webhook folder](/img/figure-html/share_folder.png)

# Set up Google Sheets to listen to webhook notifications
Following the helpful guide in the [Runscope Blog](https://blog.runscope.com/posts/tutorial-capturing-webhooks-with-google-sheets), I was able to use a [Google Sheet](https://www.google.com/sheets/about/) as a web app to listen to webhook notifications. Once I set up the app and obtained the Google Sheet URL, I was ready to create the webhook.

# Create webhook for folder
I used this [Box guide](https://developer.box.com/guides/webhooks/manage/for-folder/#webhook-triggers) as a starting point to set up the webhook for folder using Python. 

You will need the following information to set up webhook:
...
Now that webhook has been set up, it is time to see what the notification looks like in Google Sheets. After uploading a blank PDF to any subfolder of my root folder (folder set up for webhook), this is the notification:

![Webhook notification in Google Sheets](/img/webhook_response.png)

In order to parse the JSON text in notification, I used R. For my project, I needed the file name and URL, but the script could be adapted for information of interest from JSON notification:
